...
if (typ=='norm') or (typ=='nr-day'):
    ...
    fpagg=nc.Dataset(odir+'all/prerain1.nc','r')
i=0
...
for var in vars:
    ...
    if typ=='norm':
        ...
        if typ=='nr-fg':
            ...
            nm='t'
        if var=='EF':
            datat=fpagg[nm+'LH_het'][:]/(fpagg[nm+'HFX_het'][:]+fpagg[nm+'LH_het'][:])
            datag=fpagg[nm+'LH_hmg'][:]/(fpagg[nm+'HFX_hmg'][:]+fpagg[nm+'LH_hmg'][:])
        else:
            datag=fpagg[nm+var+'_hmg'][:]
            datat=fpagg[nm+var+'_het'][:]
...
#### CREATE OUTPUT ####
try:
    fp=nc.Dataset(odir+'all/time_stats_'+typ+'.nc','r+')
except Exception:
    fp=nc.Dataset(odir+'all/time_stats_'+typ+'.nc','w')
